...
class ICASMTrainerComponent:
    ...
    def train(self, epoch, trainer):
        ...
        for e in range(epoch):
            for i, (source_data, source_labels, target_data) in enumerate(
                ...
                discriminator_loss, accuracy = self._train_supervised(source_data, source_labels)
                discriminator_loss, generator_loss, classification_loss = self._train_source_modeling(source_data, source_labels)
                trainer.experiment.log_metric('D(x)', discriminator_loss.item())
                trainer.experiment.log_metric('D(G(x))', generator_loss.item())
                trainer.experiment.log_metric('NLL G(x)', classification_loss.item())
            ...
            print("Epoch: {} Acc: {} D(x): {} D(G(x)): {} NLL G(x): {}".format(
    ...
    def _train_supervised(self, source_data, source_labels):
        ...
        source_preds = self.trainer.model.classifier.prob(source_features)
    ...
    def _train_source_modeling(self, source_data, source_labels):
        ...
        rm_source_features = self._rm_map(source_features)
        true_preds = self.trainer.model.source_discriminator(rm_source_features.detach())

        rm_source_fake_features = self._rm_map(source_fake_features)
        fake_preds = self.trainer.model.source_discriminator(rm_source_fake_features.detach())
        ...
        rm_source_fake_features = self._rm_map(source_fake_features)
        fake_preds = self.trainer.model.source_discriminator(rm_source_fake_features)
        true_labels = torch.ones(fake_preds.size(0)).long().to(self.trainer.device)
        generator_loss = F.nll_loss(fake_preds, true_labels)

        source_fake_preds = self.trainer.model.classifier.prob(source_fake_features)
        classification_loss = F.nll_loss(source_fake_preds, source_labels)
