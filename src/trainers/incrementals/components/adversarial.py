...
class IncrementalAdversarialTrainer:
    ...
    def __init__(self,
        ...
        self.classifier_optim = optim.SGD(
            self.model.classifier.parameters(), lr=1e-3)
        self.source_optim = optim.Adam(
            self.model.source_encoder.parameters(), lr=1e-3)
        self.target_optim = optim.Adam(
            self.model.target_encoder.parameters(), lr=1e-4)
        self.discrim_optim = optim.Adam(
            self.model.domain_discriminator.parameters(), lr=1e-4)
        self.source_domain_discriminator_optim = optim.Adam(
            self.model.source_discriminator.parameters(), lr=1e-4)
        self.source_domain_generator_optim = optim.Adam(
            self.model.source_generator.parameters(), lr=1e-4)
