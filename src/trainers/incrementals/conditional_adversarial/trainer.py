class IncrementalConditionalAdversarialTrainer:
    ...
    def __init__(self,
        ...
        self.randomized_f = torch.randn(feature_dim, args.dim_features).detach()
    ...
    def validate(self, e):
        ...
        for i, (target_data, target_labels) in enumerate(self.validate_data_loader):
            ...
            accuracy += self.z_dim * (target_preds == target_labels).sum().item() / target_preds.size()[0]
    ...
    def _train_source_modeling(self, source_data):
        ...
        true_preds = self.model.source_domain_discriminator(source_mul_features.detach())

        z = torch.randn(self.batch_size, self.z_dim).to(self.device).detach()
        source_fake_features = self.model.source_generator(z)
        source_fake_mul_features = self._randomized_multilinear_map(source_fake_features)
        fake_preds = self.model.source_domain_discriminator(source_fake_features.detach())
        labels = torch.cat((torch.ones(self.batch_size).long().to(self.device), torch.zeros(self.batch_size).long().to(self.device)))
        ...
        source_fake_mul_features = self._randomized_multilinear_map(source_fake_mul_features)
        fake_preds = self.source_domain_discriminator(source_fake_mul_features)
        generator_loss = - F.nll_loss(fake_preds, torch.zeros(self.batch_size).long().to(self.device))
    ...
    def _ad_train_target_encoder(self, target_data):
        ...
        target_mul_features = self._randomized_multilinear_map(target_features.detach())
        target_domain_predicts = self.model.domain_discriminator(target_mul_features)
        target_adversarial_loss = - F.nll_loss(target_domain_predicts, torch.zeros(self.batch_size).long().to(self.device))
    ...
    def _ad_train_discriminator(self, source_data, target_data):
        ...
        target_features = self.model.target_encoder(target_data)
        target_mul_features = self._randomized_multilinear_map(target_features.detach())
        target_domain_preds = self.model.domain_discriminator(target_mul_features)

        domain_labels = torch.cat((torch.ones(self.batch_size).long().to(self.device), torch.zeros(self.batch_size).long().to(self.device)))
